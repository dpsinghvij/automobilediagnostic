...
from factorout import FactorDisp, FactorWithName
    ...
    for i in range(len(user_evidence)):
        ...
        for j in range(len(nodes)):
            ...
            if nodes[j] == user_evidence[i]:
                ...
                #data=json.dumps(convertFactorToDict(q[nodes[j]]), default=obj_dict)
                output.append(FactorWithName(nodes[j],convertFactorToDict(q[nodes[j]])))
              #  print(.encode(convertFactorToDict(q[nodes[j]]),unpicklable=False))
        print(json.dumps(output,default=obj_dict))
...
def getAskedProbability(user_evidence, query_variable):
    ...
    for i in range(len(user_evidence)):
        for j in range(len(query_variable)):
            ...
            output.append(FactorWithName(query_variable[j], convertFactorToDict(q[query_variable[j]])))
